...
#         lhs[-1, -2] = 1
        ...
        b_squared = (Const.I**2 / (np.pi * Const.r_0**2 * Const.c**2) 
                     * (2 / (self.p_exp * self.sys.grid.rr**2) * gamma(2 / self.p_exp) * gammainc(2 / self.p_exp, self.sys.grid.rr**self.p_exp) 
                     - np.exp(-self.sys.grid.rr**self.p_exp)))
...
#         
#         rho = norm * f1(rho[1:i])
#         B_theta = norm * f1(B_theta[1:i])
#         V_r = norm * f1(V_r[1:i])
#         V_z = norm * f2(V_z[1:i])
#         p = norm * f1(p[1:i])    
#         temp = norm * (f1(temp[1:i]) - 1)
#         rr_csv = rr[1:i]
#         
#         np.savetxt('rr.csv', rr_csv, delimiter=',')
#         np.savetxt('rho_ideal.csv', rho, delimiter=',')
#         np.savetxt('B_theta_ideal.csv', B_theta, delimiter=',')
#         np.savetxt('V_r_ideal.csv', V_r, delimiter=',')
#         np.savetxt('V_z_ideal.csv', V_z, delimiter=',')
#         np.savetxt('p_ideal.csv', p, delimiter=',')
#         np.savetxt('T_ideal.csv', temp, delimiter=',')
...
#                 r[1: -1], f2(V_theta[1: -1]))
        ...
        B_r_contour = f1(z_osc * B_r)
        B_theta_contour = B_0.T + f1(z_osc * B_theta)
        B_z_contour = (B_Z0 * np.ones(nr)).T + f1(z_osc * B_z)
...
#         rr = rr * rr.T
#         rr_csv = rr[1:-1, 1:i]
#         np.savetxt('rr_c.csv', rr_csv, delimiter=',')
#         np.savetxt('rho_ideal_c.csv', rho_contour[1:-1, 1:i], delimiter=',')
#         np.savetxt('B_theta_ideal_c.csv', B_theta_contour[1:-1, 1:i], delimiter=',')
#         np.savetxt('V_r_ideal_c.csv', V_r_contour[1:-1, 1:i], delimiter=',')
#         np.savetxt('V_z_ideal_c.csv', V_z_contour[1:-1, 1:i], delimiter=',')
#         np.savetxt('p_ideal_c.csv', p_contour[1:-1, 1:i], delimiter=',')
#         np.savetxt('T_ideal_c.csv', temp_contour[1:-1, 1:i], delimiter=',')
...
#         Vr_con = V_r_contour[1:-1, 1:i]
#         Vz_con = V_z_contour[1:-1, 1:i]
#         np.savetxt('Vr_quiver.csv', Vr_con[::d_vec, ::d_vec], delimiter=',')
#         np.savetxt('Vz_quiver.csv', Vz_con[::d_vec, ::d_vec], delimiter=',')
