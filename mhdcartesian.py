...
class NumericalEquilibriumCar:
    def __init__(self, sys, rho, g):
    ...
    def compute_fields_from_rho(self):
        ...
        rho0 = self.rho
        p0 = cumtrapz(self.g * rho0, self.sys.grid_r.r, initial=0)
...
class LinearCar:
    def __init__(self, equ, k=1, m=0, rosh=5/3, bc_array=None):
        self.equ = equ
        self.k = k
        self.m = m
        self.rosh = rosh
    ...
    def construct_operator(self, k=1, m=0):
        ...
        m_p_Vr = fd.diag(-1j * (fd.ddr(1) @ p)) - rosh * 1j * p * fd.ddr(1)
    ...
    def extract_from_evec(self, i=-1, epsilon=0.05):
        ...
        Br     = np.zeros(nr) #epsilon * phase * v_omega[1*nr: 2*nr]
        Btheta = epsilon * phase * v_omega[1*nr: 2*nr]
        Bz     = np.zeros(nr) #epsilon * phase * v_omega[3*nr: 4*nr]
        Vr     = epsilon * phase * v_omega[2*nr: 3*nr]
        Vtheta = np.zeros(nr) #epsilon * phase * v_omega[5*nr: 6*nr]
        Vz     = epsilon * phase * v_omega[3*nr: 4*nr]
...
#         T   = T.T
    ...
    def evolve_Euler(self, courant=0.8, t_max=0):
        ...
        while t < t_max:
            ...
            rho[1: -1, 1: -1] = (rho_temp[1: -1, 1: -1] - dt / (2 * dr) * (rho_temp[2: , 1: -1] * Vr_temp[2: , 1: -1] 
                                                        - rho_temp[: -2, 1: -1] * Vr_temp[: -2, 1: -1])
            ...
            p[1: -1, 1: -1] = (p_temp[1: -1, 1: -1] - dt / (2 * dr) * Vr_temp[1: -1, 1: -1] * (p_temp[2: , 1: -1] - p_temp[: -2, 1: -1]) 
                               ...
                               - rosh * dt / (2 * dr) * p_temp[1: -1, 1: -1] * (Vr_temp[2: , 1: -1] - Vr_temp[: -2, 1: -1])
                               ...
                               + (1 / (2 * dr) * (B_temp[2:, 1: -1] - B_temp[:-2, 1: -1]))**2))
    ...
    def evolve_WENO(self, courant=0.8, t_max=0):
        ...
        while t < t_max:
            ...
            B   = PDESolverCar(B_temp,   fluxR_B,   fluxZ_B,   source_B,   self).time_step()
            Ur  = PDESolverCar(Ur_temp,  fluxR_Ur,  fluxZ_Ur,  source_Ur,  self).time_step()
            Uz  = PDESolverCar(Uz_temp,  fluxR_Uz,  fluxZ_Uz,  source_Uz,  self).time_step()
            rho = PDESolverCar(rho_temp, fluxR_rho, fluxZ_rho, source_rho, self).time_step()
            e   = PDESolverCar(e_temp,   fluxR_e,   fluxZ_e,   source_e,   self).time_step()
